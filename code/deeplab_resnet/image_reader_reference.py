...
IMG_MEAN = np.array((104.00698793,116.66876762,122.67891434, 113.66572732, 113.66572732), dtype=np.float32)
...
def read_images_from_disk(input_queue, input_size, random_scale): # optional pre-processing arguments
    ...
    img = tf.image.decode_jpeg(img_contents, channels=5)
    img_r, img_g, img_b, img_p, img_n = tf.split(axis=2, num_or_size_splits=5, value=img)
    img = tf.cast(tf.concat(2, [img_b, img_g, img_r, img_p, img_n]), dtype=tf.float32)
    # extract mean
    img -= IMG_MEAN
